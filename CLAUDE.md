...
**If you touched `web/`, those four gates say NOTHING about it** — `pytest` never
loads it and `ruff` never reads TypeScript. It has four of its own, and the last one
is not redundant:

```bash
cd web && npx tsc --noEmit    # exit 0
npm run lint                   # 0 problems
npx vitest run                 # green — READ THE FILE COUNT, not the test count
npx next build                 # 7 routes
```

Two defects passed `tsc` and `eslint` and were caught only by `next build`; a third
(`/` answering 404) only by serving it. See the web section for both.
